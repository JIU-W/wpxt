...
public class FileInfoServiceImpl implements FileInfoService {
    ...
    public UploadResultDto uploadFile(SessionWebUserDto webUserDto, String fileId, MultipartFile file,
        ...
        try {
            ...
            if (chunkIndex == 0) {
                ...
                infoQuery.setSimplePage(new SimplePage(0, 1));//代表只查第一条
                infoQuery.setStatus(FileStatusEnums.USING.getStatus());
                List<FileInfo> dbFileList = this.fileInfoMapper.selectList(infoQuery);
                //通过md5值在数据库中查到了文件，则直接 秒传
                if (!dbFileList.isEmpty()) {
                    FileInfo dbFile = dbFileList.get(0);
                    //判断文件状态，大小   如果该文件大小加已使用文件大小 > 总空间大小，直接抛异常
    ...
    public void transferFile(String fileId, SessionWebUserDto webUserDto) {
        ...
        try {
            ...
            //找到临时目录fileFolder,分片好的若干文件就在这个路径下面
            String tempFolderName = appConfig.getProjectFolder() + Constants.FILE_FOLDER_TEMP;
            String currentUserFolderName = webUserDto.getUserId() + fileId;
            ...
            if (FileTypeEnums.VIDEO == fileTypeEnum) {
                ...
                //图片的宽度小于等于指定的缩略图宽度，不需要进行压缩，直接把原图片复制一份作为缩略图
                if (!created) {
    ...
    public static void union(String dirPath, String toFilePath, String fileName, boolean delSource) throws BusinessException {
        ...
        //RandomAccessFile：支持随机访问文件的类，
        //可以直接跳转到文件的任意位置进行读写，而不像其他流式操作那样按顺序读写。
        ...
        try {
            ...
            for (int i = 0; i < fileList.length; i++) {
                ...
                try {
                    ...
                    while ((len = readFile.read(b)) != -1) {//读文件
                        //写到目标文件
    ...
    private void cutFile4Video(String fileId, String videoFilePath) {
        ...
        //生成 索引文件.m3u8 和 切片.ts
        //将TS格式的视频文件切割成时长为30秒的小片段，生成的小片段保存在同一文件夹中，并生成一个M3U8播放列表文件。
     ...
     * @return
     */
    @Transactional(rollbackFor = Exception.class)     //从前端传过来的fileName只是文件名，不带后缀名
    public FileInfo rename(String fileId, String userId, String fileName) {
        ...
        //再次校验文件名  但是叫这个的话就要加上事务，让这一步和上一步的修改文件名放在一个事务里，有异常则一起回滚
        FileInfoQuery fileInfoQuery = new FileInfoQuery();
        fileInfoQuery.setFilePid(filePid);
        fileInfoQuery.setUserId(userId);
        fileInfoQuery.setFileName(fileName);
    ...
    private void checkFileName(String filePid, String userId, String fileName, Integer folderType) {
        FileInfoQuery fileInfoQuery = new FileInfoQuery();  //folderType:   0:文件   1:目录
    ...
    public void changeFileFolder(String fileIds, String filePid, String userId) {
        ...
        //转成Map集合格式   文件名作为 key,文件作为 value
        Map<String, FileInfo> dbFileNameMap = dbFileList.stream().collect(Collectors.toMap(
        ...
        for (FileInfo item : selectFileList) {
            //用要移动的文件的文件名给  目标目录去查找，若不为空，则说明目标目录中有原始文件和要移动的文件重名
    ...
    public void recoverFileBatch(String userId, String fileIds) {
        ...
        //找到所有的 要恢复的目录 并把它们的id存入 delFileSubFolderFileIdList
        for (FileInfo fileInfo : fileInfoList) {
        ...
        //查询所有所选文件
        //将 要恢复的目录 下的所有删除的文件更新为正常 delFileSubFolderFileIdList  要恢复的文件的父id都是上一步查到的文件夹id
     ...
     * @param adminOp
     */
    @Transactional(rollbackFor = Exception.class)   //adminOp表示是不是管理端进行的删除操作
